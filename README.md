...
Generative AI is basically a type of machine learning where the model doesn't just predict a label or a number, it actually creates new content like text, images, audio, code, video etc. The way it works is the model looks at a huge amount of data during training and picks up on the underlying patterns in it, and once it's learned that, it can sample from what it learned to produce something new that wasn't in the training set but still "fits" the patterns it saw. Most of today's GenAI (LLMs, image generators, etc.) is built on top of the Transformer architecture, trained on massive datasets.

**How it's different from traditional ML:**
Normal/traditional ML is mostly discriminative, meaning it learns a fixed mapping from input to output. Like, is this email spam or not, or what's the predicted price of this house. It's answering a "what is this" type questions. GenAI flips that around and answers more of a "what could this be" question, it makes new data that's statistically similar to what it trained on, but not copied from it. Also traditional ML usually needs labeled data for one specific task, while GenAI models are pretrained on tons of unlabeled data and can then be used for a bunch of different tasks afterward.

**3 real-world applications:**
- **Chatbots / text generation** — LLMs like GPT power things like ChatGPT, writing assistants, summarizers, code helpers, etc.
- **Image generation** — tools like Stable Diffusion or DALL and Googles Nano Banana can generate a full image just from a text prompt, used a lot in design and marketing now.
...
| **Typical use case** | Understanding tasks like classification, embeddings, encoding context (e.g., BERT) | Generation tasks like text generation, translation output (e.g., GPT, decoder side of T5) |
